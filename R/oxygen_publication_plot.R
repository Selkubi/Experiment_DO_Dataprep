...
modelsummary::modelsummary(model_power, stars = TRUE, shape = term ~ model + statistic, statistic = "std.error", fmt = 2, 
                           output = "output/model.csv")
readr::write_csv(as.data.frame(pairwise_comparisons) %>%
                   mutate(across(where(is.numeric), ~round(., 4))), "output/pairwise_comparisons.csv")
